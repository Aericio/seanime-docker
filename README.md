...
Unlike umag's version, the server image is based on `nvidia/cuda:13.0.2-base-ubuntu24.04` image instead of alpine images. This is because alpine images do not support CUDA/NVENC, so hardware transcode will not work. To verify that the container has CUDA/NVENC support with FFMPEG, run `ffmpeg -hide_banner -init_hw_device "list"` and `cuda` should be an option.
